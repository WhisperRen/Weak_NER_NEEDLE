...
class CRF(nn.Module):
    ...
    def decode(self, emissions: torch.Tensor,
               mask: Optional[torch.ByteTensor] = None) -> Tuple[List[List[int]], List[float]]:
        ...
        Tuple[List[List[int]], List[float]]
            List of list containing the best tag sequence for each batch; List of max decode score for each batch
        """
    ...
    def _viterbi_decode(self, emissions: torch.FloatTensor,
                        mask: torch.ByteTensor) -> Tuple[List[List[int]], List[float]]:
        # emissions: (seq_length, batch_size, num_tags)
        # mask: (seq_length, batch_size)
        assert emissions.dim() == 3 and mask.dim() == 2
        assert emissions.shape[:2] == mask.shape
        ...
        for idx in range(batch_size):
            ...
            best_last_score, best_last_tag = score[idx].max(dim=0)
            best_scores_list.append(best_last_score.item())
        ...
        return best_tags_list, best_scores_list
...
class AlbertForNER(AlbertPreTrainedModel):
    ...
    def forward(self,
        ...
        if labels is not None:
            score = self.crf(emissions=logits,
                             tags=labels,
                             mask=attention_mask.to(torch.bool),
                             reduction=crf_reduction)
            outputs = (-1 * score,) + outputs
